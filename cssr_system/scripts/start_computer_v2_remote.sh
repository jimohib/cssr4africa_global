...
################################################################################
# CSSR System - Computer Startup Script (Version 2: Computer as Remote Node)
#
# This script launches the CSSR system nodes on the main computer.
# In Version 2, the Computer connects to the Jetson's ROS MASTER.
#
# It handles:
...
#
# Arguments:
#   JETSON_IP           - IP address of Jetson running roscore (default: 172.29.111.240)
...
# Launch file path
LAUNCH_FILE="$(rospack find cssr_system 2>/dev/null || echo "$ROS_WORKSPACE/src/cssr4africa_global/cssr_system")/launch/cssrSystemLaunchComputer_v2_remote.launch"
...
print_header "CSSR System - Computer Startup (Version 2: Remote Node)"
...
if timeout 5 rostopic list &> /dev/null; then
    ...
    print_info "  1. Jetson is running (start_jetson_v2_master.sh)"
...
if timeout 5 rostopic list 2>/dev/null | grep -q "/faceDetection"; then
    ...
    print_info "  1. Started the Jetson nodes using start_jetson_v2_master.sh"
...
print_info "Note: roscore is still running on Jetson"
